...
import { defaultProject, makeTextLayer, makeImageLayer } from '../engine/project.js';
...
export default function App() {
  ...
  const onFile = e => {
    ...
    const img = new Image();
    img.onload = () => {
      const layer = makeImageLayer(img, { name: file.name, start: +player.t.toFixed(2) });
      setProject(p => ({ ...p, layers: [...p.layers, layer] }));
      setSelectedId(layer.id);
      setTab('props');
    };
  ...
  return (
    <div className="app">
      <header className="topbar">
        ...
        <button className="btn" onClick={() => fileRef.current.click()}>+ IMAGEM</button>
