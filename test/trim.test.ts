...
import { trimLeft, trimRight, maxDuration, MIN_CLIP } from '../src/engine/project.ts';
import { sourceTimeAt, isLayerActive } from '../src/engine/videoSync.ts';

import type { TrimTarget } from '../src/engine/project.ts';

/** A 10s source, placed at t=2, showing seconds 1..5 of the file. */
const clip = (): TrimTarget => ({
  start: 2,
  duration: 4,
  trimStart: 1,
  sourceDuration: 10,
...
const textLayer = (): TrimTarget => ({ start: 2, duration: 4, sourceDuration: Infinity });
